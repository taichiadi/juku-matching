...
export default async function CorrectionPage() {
  ...
  const used = count ?? 0;
  const { allowed, limit, remaining } = canUseService(plan, "corrections", used);
    ...
    <div>
      {remaining !== null && remaining <= 1 && (
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-2 text-center text-xs font-black text-amber-700">
          今月の添削残り {remaining} 回
          <Link href="/student/plan?upgrade=pro" className="ml-2 underline">プランを見る</Link>
        </div>
      )}
      <StudentServicePageView kind="correction" />
    </div>
  );
}
