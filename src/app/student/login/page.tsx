      ...
      <main className="mx-auto max-w-5xl px-4 py-5 md:py-10">
        <div className="grid gap-5 lg:grid-cols-[1fr_380px] lg:items-start">
          ...
          <div className="order-last lg:order-first">
            <p className="text-xs font-black tracking-[0.36em] text-lime-600">STUDENT ACCOUNT</p>
            <h1 className="mt-2 text-xl font-black leading-tight md:text-3xl">
              ログインすると、<span className="text-cyan-600">これができる。</span>
            </h1>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {FEATURES.map((feature) => (
                ...
                >
                  ...
                  <p className="mt-2 text-xs font-black text-slate-900">{feature.label}</p>
                  <p className="mt-0.5 text-xs leading-5 text-slate-600">{feature.body}</p>
                </div>
              ))}
            </div>
            ...
            {selectedService && (
              <div className="mt-3 rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2.5">
                <p className="text-xs font-black tracking-[0.28em] text-cyan-700">{selectedService.label}</p>
                <p className="mt-1 text-xs font-black text-slate-900">{selectedService.title}</p>
                <p className="mt-0.5 text-xs leading-5 text-slate-600">{selectedService.body}</p>
              </div>
            )}

            {/* Free features notice */}
            <div className="mt-3 flex flex-wrap gap-1.5">
              {["体験記はログイン不要", "相談はログイン後・単発1,500円〜"].map((item) => (
